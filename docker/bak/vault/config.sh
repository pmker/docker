...
awk 'NR<=5{print "curl --request PUT --data \"" $0 "\" hashserver_consul.hashserver_genv:8500/v1/kv/vault" NR-1 + 1}' init >> consulStoring.sh
awk 'END {print "curl --request PUT --data \"" $0 "\" hashserver_consul.hashserver_genv:8500/v1/kv/vaultLogin"}' init >> consulStoring.sh
...
awk 'NR==3{print "curl --request PUT --data \"" $2 "\" hashserver_consul.hashserver_genv:8500/v1/kv/appToken"}' policy >> consulStoring.sh
